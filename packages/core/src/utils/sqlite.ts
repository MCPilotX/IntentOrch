...
class SqliteProcessRepository implements IProcessRepository {
  ...
  async upsert(process: Record<string, unknown>): Promise<void> {
    ...
    await db.execute(
      ...
      [
        process.pid ?? 0,
        process.server_name as string ?? "",
        process.name as string ?? "",
        process.version as string ?? "",
        process.manifest as string ?? "{}",
        process.start_time as number ?? Date.now(),
        process.status as string ?? "running",
        process.port as number ?? null,
        process.log_path as string ?? null,
        process.external as number ?? 0,
        process.transport_type as string ?? null,
        process.url as string ?? null,
        process.tools as string ?? null,
      ],
    );
  }
