...
export class IntentService {
      ...
      if (!this.initPromise) {
        this.initPromise = (async () => {
          if (this.aiConfig.apiKey) {
            logger.info(
              `[IntentService] Configuring AI with provider: ${this.aiConfig.provider || "openai"}, model: ${this.aiConfig.model || "gpt-3.5-turbo"}`,
            );
          }
  ...
  private async convertToWorkflowSteps(
    plan: any,
    context?: any,
    ...
    for (const planStep of plan.steps) {
      ...
      if (planStep.toolName) {
        ...
        const step: WorkflowStep = {
          ...
          parameters: await this.adaptParameters(planStep.arguments || {}, {
            name: planStep.toolName,
          }),
        };
  ...
  private normalizeServerName(serverName: string): string {
    ...
    const normalized = serverName.replace(/^(github:|gitee:|gitlab:)/, "");
  ...
  private async extractServerId(
    ...
    try {
      ...
      if (allTools && Array.isArray(allTools)) {
        const toolMetadata = allTools.find(
      ...
      logger.warn(
        `[IntentService] Failed to get server from registry for tool "${toolName}":`,
        error,
      );
    }
    ...
    if (context?.availableServers && context.availableServers.length > 0) {
      for (const server of context.availableServers) {
        if (
          ...
          server.includes(toolName.split(".")[0])
        ) {
    ...
    try {
      ...
      if (toolMetadata && toolMetadata.parameters) {
        ...
        for (const [intentKey, intentValue] of Object.entries(intentParams)) {
          ...
          for (const [schemaKey, paramSchema] of Object.entries(actualSchema)) {
            ...
            if (
              ...
              paramInfo.description
                .toLowerCase()
                .includes(intentKey.toLowerCase())
            ) {
              adapted[schemaKey] = intentValue;
              mapped = true;
              break;
        ...
        for (const [key, value] of Object.entries(adapted)) {
          if (
            typeof value === "string" &&
            (key.toLowerCase().includes("date") || this.looksLikeDate(value))
          ) {
        ...
        const processed = ParameterPostProcessor.process(adapted, {
          properties: actualSchema,
        });
        logger.info(
          `[IntentService] Final adapted parameters for ${toolName}:`,
          JSON.stringify(processed.params),
        );
  ...
  private ensureRequiredParameters(
    ...
    for (const [paramName, paramSchema] of Object.entries(actualSchema)) {
      if ((paramSchema as any).required && !adaptedParams[paramName]) {
        logger.warn(
          `[IntentService] Missing required parameter: "${paramName}" for tool "${toolName}"`,
        );
      }
    }
  }
