...
import {
  toLightweightManifest,
  supportsDynamicDiscovery,
  ...
  const command = new Command("pull")
    ...
    .action(async (server: string) => {
      try {
        ...
        if (cachedManifest) {
          console.log(
            `ℹ️  Configuration for ${displayName} v${cachedManifest.version} is already cached`,
          );
          manifest = cachedManifest;
        } else {
          manifest = await registryClient.fetchManifest(server);
          console.log(
            `✓ Pulled configuration for ${displayName} v${manifest.version}`,
          );
        }
        ...
        if (supportsDynamic) {
          console.log(
            `  Tool discovery: Will discover tools dynamically when server starts`,
          );
          ...
          const hasValidTools = (tools: any[]) =>
            tools && tools.length > 0 && tools.some((t) => t.name);

          const manifestTools = manifest.tools || [];
          const capabilitiesTools = manifest.capabilities?.tools || [];
          const hasValidStaticTools =
            hasValidTools(manifestTools) || hasValidTools(capabilitiesTools);

          if (hasValidStaticTools) {
            console.log(
              `  Tool discovery: Using static tool definitions from manifest`,
            );
            ...
            const validTools =
              manifestTools.filter((t: any) => t.name).length > 0
                ? manifestTools.filter((t: any) => t.name)
                : capabilitiesTools.filter((t: any) => t.name);
            console.log(
              `  Tools: ${validTools.length} tools registered (static)`,
            );
        ...
        console.log(`\n💡 Next steps:`);
        console.log(
          `   1. Set any required secrets: intorch secret set <name> <value>`,
        );
        console.log(`   2. Start the server: intorch start ${displayName}`);
        console.log(
          `   3. Tools will be automatically discovered after server starts`,
        );
      } catch (error) {
        console.error(
          `Failed to pull ${getDisplayName(server)}:`,
