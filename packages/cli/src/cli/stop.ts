...
export function stopCommand(): Command {
  const command = new Command("stop")
    ...
    .argument(
      "<target>",
      "Process ID or Server name (e.g., 1234 or Joooook/12306-mcp)",
    )
    ...
    .action(async (target: string, options) => {
      try {
        ...
        if (!isNaN(parsedPid) && parsedPid.toString() === target) {
          ...
          const matchingServers = runningServers.filter(
          ...
          if (matchingServers.length === 0) {
            ...
            const stoppedProcesses = allProcesses.filter(
              (s) => s.serverName === serverName && s.status === "stopped",
            );
