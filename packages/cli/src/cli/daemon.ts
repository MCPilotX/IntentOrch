...
async function startDaemonDetached(host: string, port: string): Promise<void> {
  ...
  const child = spawn(
    process.execPath,
    [scriptPath, "daemon", "start", "--host", host, "--port", port],
    {
      detached: true,
    ...
    .action(async () => {
      try {
        ...
        if (await client.isDaemonRunning()) {
          ...
          console.log(
            `URL: http://${status.config.host}:${status.config.port}`,
          );
          ...
          if (isRunning && pid) {
            ...
            console.log(`PID: ${pid}`);
            console.log(
              `Note: Daemon process exists but HTTP server may not be accessible`,
            );
          } else {
